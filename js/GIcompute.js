...
function computeLayers(){
	...
	for(var i=0;i<isGeometries.length;i++){
		// removePointWhichEqualGradient( isGeometries[i]);
	}
	//断面の表示
	for(var i=0;i<isGeometries.length;i++){
...
function removePointWhichEqualGradient( geometry_ ){

	for(var i=0;i<geometry_.length - 2;i++){

		var grad1 = (geometry_.vertices[i].z - geometry_.vertices[i+1].z)/(geometry.vertices[i].x / geometry_.vertices[i+1].x);
		var grad2 = (geometry_.vertices[i+1].z - geometry_.vertices[i+2].z)/(geometry.vertices[i+1].x / geometry_.vertices[i+2].x);

		if(Math.abs(grad1 - grad2) < 0.0000001){
			geometry_.vertices.splice(i+1, 1);
			i--;
		}
	...
	// var flag = true;
	// while(flag){
	// 	var grad1 = (geometry_.vertices[geometry_.vertices.length-2].z - geometry_.vertices[geometry_.vertices.length-1].z)/(geometry_.vertices[geometry_.vertices.length-2].x / geometry_.vertices[geometry_.vertices.length-1].x);
	// 	var grad2 = (geometry_.vertices[geometry_.vertices.length-1].z - geometry_.vertices[0].z)/(geometry_.vertices[geometry_.vertices.length-1].x / geometry_.vertices[0].x);
    //
	// 	if(Math.abs(grad1 - grad2) < 0.0000001){
	// 		geometry_.vertices.splice(geometry_.vertices.length -1 , 1);
	// 	}
	// 	console.log("while");
	// 	flag = false;
	// }
    //
	// while(flag){
	// 	var grad1 = (geometry_.vertices[geometry_.vertices.length-2].z - geometry_.vertices[geometry_.vertices.length-1].z)/(geometry.vertices[geometry_.vertices.length-2].x / geometry_.vertices[geometry_.vertices.length-1].x);
	// 	var grad2 = (geometry_.vertices[0].z - geometry_.vertices[1].z)/(geometry.vertices[0].x / geometry_.vertices[1].x);
    //
	// 	if(Math.abs(grad1 - grad2) < 0.0000001){
	// 		geometry_.vertices.splice(i+1, 1);
	// 	}
...
var toleranceError = 0.1;

function isEqualVector3( v, w){

	if(! Math.abs(v.x - w.x) < toleranceError ) return false;
	if(! Math.abs(v.y - w.y) < toleranceError ) return false;
	if(! Math.abs(v.z - w.z) < toleranceError ) return false;
